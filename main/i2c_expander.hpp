...
class I2C_expander: public I2C::dev {
public:
	I2C_expander() {
		addr = 42;
	}
	...
	uint32_t read_uid() {
		return read_reg<uint32_t>(0x00);
	}
	void reset() {
		write_reg(0x01);
	}
	void port_mode_input(uint16_t pins) {
		write_reg(0x04, pins);
	}
	void port_mode_pullup(uint16_t pins) {
		write_reg(0x05, pins);
	}
	void port_mode_pulldown(uint16_t pins) {
		write_reg(0x06, pins);
	}
	void port_mode_output(uint16_t pins) {
		write_reg(0x07, pins);
	}
	void digital_write_high(uint16_t pins) {
		write_reg(0x09, pins);
	}
	void digital_write_low(uint16_t pins) {
		write_reg(0x0A, pins);
	}
	uint16_t digital_read() {
		read_reg<uint16_t>(0x08); // срабатывает только со второго раза
		return read_reg<uint16_t>(0x08);
	}
};
